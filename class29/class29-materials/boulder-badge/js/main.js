...
function canCharmanderBattle(currentCelTemp){
    if(currentCelTemp>0){
        return `Charmander will battle`
    }else{
        return `Charmander will not battle`
    }
}
console.log(canCharmanderBattle(fahrenheitToCel(60)))
//so log the return of can charmanderbattle using the return value from converting a fahrenheit temp of 60.

//Loops
//You have joined an undeground pokemon leauge. In this league, trainers can use any number of pokemon. Print to the console "Pikachu I choose you" x times where x is the number of pokemon the trainer you are battling has in their party
let enemyTeam = ['pikachu', 'weedle']
for(let i = enemyTeam.length; i>0; i--){
